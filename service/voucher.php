...
function getVoucherUserMerchentDetail($vid){
  //  $sql = "SELECT * FROM vouchers, offers, users where vouchers.offer_id = offers.id and vouchers.user_id = users.id and vouchers.id=:id";
	...
	try {
		...
		if(!empty($vouchers)){
                         $today = date('Y-m-d');
					$checkdate = date('Y-m-d', strtotime($vouchers->item_expire_date));
					if($checkdate<$today)
					{
						$status = 'Expired';
					}else{
    ...
    try {
	    ...
	    if($resale_count==0){
			...
			$allinfo['save_data']['status'] = 'resale';
                        $allinfo['save_data']['offering_start_date'] = $allinfo['save_data']['offering_start_date'].' 23:59:59';
                        $allinfo['save_data']['offering_end_date'] = $allinfo['save_data']['offering_end_date'].' 23:59:59';
	    ...
	    if(!empty($resales)){
			for($i=0;$i<$list_count;$i++){
			    ...
			    $checkdate = date('Y-m-d', strtotime($resales[$i]->expire_date));
                            $resellexpiredate = date('Y-m-d', strtotime($resales[$i]->offering_end_date));
				...
				if($checkdate<$today || $resellexpiredate<$today)
				{
...
function getResellListPostOthers($userid){
	$current_date = date('Y-m-d');
	 $resales = array();
	   	 $sql = "SELECT offers.title,offers.id as offer_id, voucher_resales.id as voucher_resale_id, voucher_resales.voucher_id, voucher_resales.price,voucher_resales.created_on, voucher_resales.points, voucher_resales.user_id, voucher_resales.is_sold, voucher_resales.is_active, vouchers.to_date as expire_date, vouchers.price as voucher_price,vouchers.offer_price as purchase_price  FROM offers, voucher_resales, vouchers WHERE offers.id = vouchers.offer_id and voucher_resales.voucher_id = vouchers.id and DATE(vouchers.item_expire_date) >=:current_date and voucher_resales.is_active ='1' and voucher_resales.is_sold='0' and voucher_resales.user_id!=:user_id";   	
    ...
    try {
	    ...
	    if(!empty($resales)){
				...
				$expire_date = date('d M, Y', strtotime($resales[$i]->expire_date));
                                if($resales[$i]->created_on == "0000-00-00 00:00:00"){
                                    $resales[$i]->created_on = "";
                                }else{
                                    $resales[$i]->created_on = date('d M, Y', strtotime($resales[$i]->created_on));
                                }
	...
	try {
		...
		if($count>0)
		{
				...
				if($checkdate<$today)
				{
                    ...
                    $offer->sharedescription = strip_tags($offer->description );
...
function getSpecialMembership() {     
	...
	$lastdate = date('Y-m-d');
	//$newdate = "2016-03-08";
        $sql = "SELECT offers.id, offers.title, offers.price, offers.offer_percent, offers.offer_from_date, offers.offer_to_date, offers.image, offers.special_tag, offers.buy_count, offers.quantity FROM offers where DATE(offers.offer_from_date) <=CURDATE() and offers.is_active=1 and offers.is_special=1 and offers.offer_to_date >=:lastdate and offers.offer_type_id=3 and offers.buy_count < offers.quantity ORDER BY offers.offer_from_date DESC";
        $site_settings = findByIdArray(1,'site_settings');
        //echo $sql;
        $site_path = SITEURL;
...
function getResellVoucherDetail($rid){
	$current_date = date('Y-m-d');
	 $resales = array();
	   	 $sql = "SELECT offers.title,offers.image, voucher_resales.id as voucher_resale_id, voucher_resales.voucher_id, voucher_resales.price,voucher_resales.created_on, voucher_resales.points, voucher_resales.user_id, voucher_resales.is_sold, voucher_resales.is_active,voucher_resales.offering_start_date,voucher_resales.offering_end_date,voucher_resales.point_id,voucher_resales.operator, vouchers.to_date as expire_date, vouchers.price as voucher_price,vouchers.offer_price as purchase_price  FROM offers, voucher_resales, vouchers WHERE offers.id = vouchers.offer_id and voucher_resales.voucher_id = vouchers.id and DATE(vouchers.item_expire_date) >=:current_date and voucher_resales.is_active ='1'  and voucher_resales.id=:r_id";   	
	   	 //$sql = "SELECT offers.title, voucher_resales.id as voucher_resale_id, voucher_resales.voucher_id, voucher_resales.price, voucher_resales.points, voucher_resales.user_id, voucher_resales.is_sold, voucher_resales.is_active, vouchers.to_date as expire_date, vouchers.price as voucher_price, max(voucher_bids.bid_price) as max_bid_price FROM offers, voucher_resales,voucher_bids, vouchers WHERE offers.id = vouchers.offer_id and voucher_resales.voucher_id = vouchers.id and voucher_bids.voucher_resale_id = voucher_resales.id and vouchers.to_date >=:current_date and voucher_resales.is_active ='1' and voucher_resales.is_sold='0' and voucher_resales.user_id!=:user_id";
    ...
    try {
	    ...
	    if(!empty($resales)){
			    ...
			    $resales[$i]->expire_date = $todate;
                            $resales[$i]->created_on = date('d M, Y', strtotime($resales[$i]->created_on));
                            if($resales[$i]->offering_end_date == "0000-00-00 00:00:00"){
                                $resales[$i]->offering_end_date = "";
                            }else{
                                $resales[$i]->offering_end_date = date('d M, Y', strtotime($resales[$i]->offering_end_date));
                            }
                            
                            if($resales[$i]->offering_start_date == "0000-00-00 00:00:00"){
                                $resales[$i]->offering_start_date = "";
                            }else{
                                $resales[$i]->offering_start_date = date('d M, Y', strtotime($resales[$i]->offering_start_date));
                            }
                            $resales[$i]->price = number_format($resales[$i]->price,1,'.',',');
                            $resales[$i]->voucher_price = number_format($resales[$i]->voucher_price,1,'.',',');
                            $resales[$i]->purchase_price = number_format($resales[$i]->purchase_price,1,'.',',');
                            $resales[$i]->image = SITEURL.'voucher_images/'.$resales[$i]->image;
                            $point_master_id = $resales[$i]->point_id;
                            if($point_master_id != 0){
                            $point_master_details = findByConditionArray(array('id'=>$point_master_id),'point_master');
                            }
                            $resales[$i]->point_master=$point_master_details;
...
function getResellPromoDetails($id,$rsid) {     
	...
	try {
		...
		if($count>0)
		{
		    ...
		    $offer->expire_date = $todate;
                    ...
                    //exit;
                    if(!empty($resellInfo)){
                        $point_master_id = $resellInfo[0]['point_id'];
                        $offer->operator = $resellInfo[0]['operator'];
                        $resell_user_id = $resellInfo[0]['user_id'];
                        $offer->resell_price = $resellInfo[0]['price'];
                        $offer->resell_point = $resellInfo[0]['points'];
                        $offer->resell_point_id = $resellInfo[0]['point_id'];
                        if($resellInfo[0]['offering_start_date'] == "0000-00-00 00:00:00"){
                            $offer->resell_start_date = '';
                        }else{
                            $offer->resell_start_date = date('m-d-Y',strtotime($resellInfo[0]['offering_start_date']));
                        }
                        if($resellInfo[0]['offering_end_date'] == "0000-00-00 00:00:00"){
                            $offer->resell_end_date = '';
                            $offer->resell_expire_date = '';
                        }else{
                    ...
                    $result = '{"type":"success","offer":'.$offer.',"restaurants":'.json_encode($restaurants).',"merchantInfo":'.json_encode($merchantInfo).',"count":'.$count.',"categories" : '.json_encode($categories).',"promo_images" : '.json_encode($offer_images).',"offer_days":'.json_encode($offer_days).',"outlets" : '.json_encode($outlets).',"point_details" : '.json_encode($point_master_details).',"resell_user_details" : '.json_encode($resellUserInfo).'}';
		}else{
			$result =  '{"type":"error","message":"Sorry no promo found"}'; 
		}
	} catch(PDOException $e) {
